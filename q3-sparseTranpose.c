...
void findTranspose(int [][3], int[][3]);
...
int main(){
	...
	printf("\nTranspose matrix \n");
	findTranspose(sparseSum,transposeMatrix);
	display(transposeMatrix,transposeMatrix[0][2]+1,3);
...
void findTranspose(int sparseSum[][3], int transposeMatrix[][3]){
	
	transposeMatrix[0][0] = sparseSum[0][1];
	transposeMatrix[0][1] = sparseSum[0][0];
	transposeMatrix[0][2] = sparseSum[0][2];
	
	int transpose_index = 1;
	int NonZeroValues = sparseSum[0][2];
	int termIndex, columnIndex ;
	

	for( columnIndex=0 ; columnIndex < sparseSum[0][1] ; columnIndex++){
		
		for( termIndex = 1 ; termIndex <= NonZeroValues ; termIndex++){
			if(sparseSum[termIndex][1]==columnIndex){

				transposeMatrix[transpose_index][0] = sparseSum[termIndex][1];
				transposeMatrix[transpose_index][1] = sparseSum[termIndex][0];
				transposeMatrix[transpose_index][2] = sparseSum[termIndex][2];
				transpose_index++;
				}
			}
		}
